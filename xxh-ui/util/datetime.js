...
/**
 * 获取当前叫早或者哄睡状态，返回night或morning
 */
export function getNowState() {
  const time = parseInt(new Date().getHours(), 10)
  let state
  if (time < 11 && time >= 5) {
    state = 'morning'
  } else {
...
export function dateRange(startDatetime, endDatetime) {
  var startTime = Date.parse(startDatetime.replace(/-/g, '/'))
  var endTime = Date.parse(endDatetime.replace(/-/g, '/'))
